...
namespace anpi {
  ...
  void  Plot2d<T>::quiver(anpi::Matrix<T>& image) {
    ...
    std::string FluxFun = "def Flux(Matriz):\n";
        ...
        FluxFun.append("	delY = 1\n");
        FluxFun.append("	if((len(Z[0])+len(Z))//(2)>=10):\n");
        FluxFun.append("		n=(len(Z[0])+len(Z))//(2*10)\n");
        FluxFun.append("	else:\n");
        FluxFun.append("		n=1\n");
        FluxFun.append("	dx = np.linspace(0,(len(Z[0])-n),len(Z[0])//n)	\n");
        FluxFun.append("	dy = np.linspace(0,(len(Z)-n),len(Z)//n)	\n");
        FluxFun.append("	X,Y = np.meshgrid(dx,dy)\n");
        FluxFun.append("	u = np.zeros((len(X),len(X[0])))\n");
        FluxFun.append("	v = np.zeros((len(X),len(X[0])))\n");
        FluxFun.append("	a,b=0,0\n");
        FluxFun.append("	for x in range(len(X)):\n");
        FluxFun.append("		for y in range(len(X[0])):\n");
        FluxFun.append("			if(a!=0 and a!=len(Z)-1 and b!=0 and b!=len(Z[0])-1):\n");
        FluxFun.append("				v[x][y]= k*(Z[a+1][b]-Z[a-1][b])/(2*delX)\n");
        FluxFun.append("				u[x][y]= -k*(Z[a][b+1]-Z[a][b-1])/(2*delY)	\n");
        FluxFun.append("			else:\n");
        FluxFun.append("				v[x][y]= 0\n");
        FluxFun.append("				u[x][y]= 0\n");
        FluxFun.append("			b+=n\n");
        FluxFun.append("		a+=n\n");
        FluxFun.append("		b=0\n");
